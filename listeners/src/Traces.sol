    ...
    function onPreCall(RawPreCallContext memory ctx) external override {
        ...
        emitTraces(
            TracesData({
                ...
                function_signature: LibString.toHexString(abi.encodePacked(bytes4(ctx.txn.call.callData()))),
                parent_function_signature: LibString.toHexString(abi.encodePacked(updateFunctionSignatures(uint64(ctx.txn.call.callDepth()), ctx.txn.call.callData()))),
                tx_from: LibString.toHexString(tx.origin),
                tx_to: LibString.toHexString(firstTxTo),
                call_type: uint64(ctx.txn.call.callType()),
                userop_from: LibString.toHexString(smartAccountSender),
